...
+ (ReceiveListCell *)cellWithTableView:(UITableView *)tableView {
    static NSString *ID = @"ReceiveListCell";
    ReceiveListCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [[ReceiveListCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:ID];
    }
    return cell;
}
...
- (void)setReceiveListOfSerialNumber:(NSString *)serialNumber type:(NSString *)type consumerName:(NSString *)consumerName commodity:(NSString *)commodity expendMoney:(NSString *)expendMoney cuttingProfits:(NSString *)cuttingProfits time:(NSString *)time status:(BOOL)status{
