...
@implementation ShoppingOrderCell

+ (ShoppingOrderCell *)cellWithTableView:(UITableView *)tableView {
    static NSString *ID = @"ShoppingOrderCell";
    ShoppingOrderCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [[ShoppingOrderCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:ID];
    }
    return cell;
}
