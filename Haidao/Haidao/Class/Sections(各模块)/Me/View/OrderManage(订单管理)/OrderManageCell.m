...
+ (OrderManageCell *)cellWithTableView:(UITableView *)tableView {
    static NSString *ID = @"OrderManageCell";
    OrderManageCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [[OrderManageCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:ID];
    }
    return cell;
}
...
- (void)setOrderManageOfSerialNumber:(NSString *)serialNumber type:(NSString *)type consumerName:(NSString *)consumerName commodity:(NSString *)commodity expendMoney:(NSString *)expendMoney cuttingProfits:(NSString *)cuttingProfits time:(NSString *)time status:(BOOL)status{
