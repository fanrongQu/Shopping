...
@interface SettingOrderViewController ()
...
/**  积分模式  */
@property (nonatomic, strong) HotboomView *integralTypeView;
/**  让利金额  */
...
@implementation SettingOrderViewController

+ (void)showSettingOrderViewController:(UIViewController *)controller {
    
    SettingOrderViewController *settingOrderVC = [[SettingOrderViewController alloc]init];
    [controller.navigationController pushViewController:settingOrderVC animated:YES];
}
...
- (void)dealloc {
    ...
    if (_integralTypeView != nil) _integralTypeView = nil;
...
- (void)addEvent {
    ...
    UITapGestureRecognizer *integralTypeViewTapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(changeIntegralType)];
    [self.integralTypeView addGestureRecognizer:integralTypeViewTapGesture];
...
- (HotboomView *)monetaryView {
    if (!_monetaryView) {
        ...
        _monetaryView.titleLabel.text = @"消费金额";
        _monetaryView.textFiled.placeholder = @"请输入消费金额";
    }
    return _monetaryView;
}

- (HotboomView *)integralTypeView {
    if (!_integralTypeView) {
        _integralTypeView = [[HotboomView alloc]init];
        [self.scrollView addSubview:_integralTypeView];
        __weak typeof(self) weakSelf = self;
        [_integralTypeView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(weakSelf.monetaryView.mas_bottom).offset(1);
            make.left.mas_equalTo(weakSelf.accountNumberView);
            make.size.mas_equalTo(weakSelf.accountNumberView);
        }];
        _integralTypeView.titleLabel.text = @"积分模式";
        _integralTypeView.detailLabel.text = @"A模式";
        
        UIImageView *imageView = [[UIImageView alloc]init];
        [_integralTypeView addSubview:imageView];
        [imageView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.mas_equalTo(-10);
            make.size.mas_equalTo(15);
            make.centerY.mas_equalTo(0);
...
- (HotboomView *)preferenceMoneyView {
    if (!_preferenceMoneyView) {
        ...
        [_preferenceMoneyView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(weakSelf.integralTypeView.mas_bottom).offset(1);
            make.left.mas_equalTo(weakSelf.accountNumberView);
            make.size.mas_equalTo(weakSelf.accountNumberView);
        }];
