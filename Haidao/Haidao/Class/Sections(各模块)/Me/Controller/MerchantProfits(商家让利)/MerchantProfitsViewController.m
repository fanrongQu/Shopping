//
//  MerchantProfitsViewController.m
//  Haidao
//
...
@interface MerchantProfitsViewController ()
...
@implementation MerchantProfitsViewController
...
- (void)viewDidLoad {
    ...
    [self setCurrentIntegral:@"10000" outputIntegral:@"2300" MerchantProfits:@"400"];
}
...
- (void)setCurrentIntegral:(NSString *)currentIntegral outputIntegral:(NSString *)outputIntegral MerchantProfits:(NSString *)MerchantProfits {
    
    self.currentIntegralLabel.text = currentIntegral;
    [self.outputButton setPresentNumber:currentIntegral type:@"送出让利额"];
    [self.punctualityButton setPresentNumber:MerchantProfits type:@"充值让利额"];
}
...
- (PresentButton *)outputButton {
    if (!_outputButton) {
        ...
        [[_outputButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
            MerchantOutputViewController *merchantOutputVC = [[MerchantOutputViewController alloc]init];
            [self.navigationController pushViewController:merchantOutputVC animated:YES];
        }];
    }
...
- (PresentButton *)punctualityButton {
    if (!_punctualityButton) {
        ...
        [[_punctualityButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
            MerchantRechargeViewController *merchantRechargeVC = [[MerchantRechargeViewController alloc]init];
            [self.navigationController pushViewController:merchantRechargeVC animated:YES];
        }];
    }
