...
- (void)buttonClick {
    ...
    //点击找回密码按钮，重置密码
    [[self.findBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
...
- (UIButton *)findBtn {
    if (!_findBtn) {
        ...
        _findBtn.titleLabel.font = kFontSize(15);
        _findBtn.backgroundColor = [UIColor redColor];
        [_findBtn setTitle:@"找回密码" forState:UIControlStateNormal];
        [_findBtn.layer setMasksToBounds:YES];
        [_findBtn.layer setCornerRadius:5.0];
