...
+ (MakeSureOrderCell *)cellWithTableView:(UITableView *)tableView {
    ...
    static NSString *ID = @"MakeSureOrderCell";
    MakeSureOrderCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
        cell = [[MakeSureOrderCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:ID];
    }
    return cell;
}
